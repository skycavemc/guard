                ...
                if (world == null) {
                    ...
                    if (args.length >= 2) {
                        ...
                        if (rg == null) {
                            ...
                            AtomicInteger villager = new AtomicInteger();
                            AtomicInteger various = new AtomicInteger();

                            world.getEntities().forEach(entity -> {
                                if (rg.contains(BukkitAdapter.asBlockVector(entity.getLocation()))) {
                                    if (entity instanceof Villager) {
                                        villager.getAndIncrement();
                                    } else if (entity instanceof LivingEntity) {
                                        living.getAndIncrement();
                                    } else if (entity instanceof Item) {
                                        item.getAndIncrement();
                                    } else {
                                        various.getAndIncrement();
                                    }
                                }
                            });
